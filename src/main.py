...
def parse_args():
    ...
    parser.add_argument('collection',type=str, help="The collection file to do the operation for")
    parser.add_argument('download_location', type=str, help="The location to download temporary files")

    parser.add_argument("--src_field", action='store',  default='Korean', type=str, help="Name of the field to get the word from")
    parser.add_argument("--dst_field", action='store',  default='Sound', type=str, help="Name of the field to write the filename to")
    parser.add_argument("--query", action='store',  default='', type=str, help="Filter which cards are downloaded for e.g 'deck:Korean'")
    parser.add_argument("--separator", action='store',  default='/', type=str, help="Used when dst contains more than one possible word")
    parser.add_argument("--driver", type=pathlib.Path, action='store', required=False, default=None)
    parser.add_argument("--debug",  action='store_true', default=False, required=False)
    parser.add_argument("--overwrite",  action='store_true', default=False, required=False)
    parser.add_argument("--browser", action='store', choices=['firefox','chrome'], default='firefox', type=str)
...
def create_handlers(args, browser):
    return [naver_tts.NaverTTS(os.path.abspath(args.download_location), browser)]
    ...
    for note_id in collection.find_notes(query):
        ...
        for retriever in handlers:
            ...
            if os.path.isfile(filepath):
                ...
                print(f"Updated note {note_id} with '{src_field}' '{note[src_field]}' with '{dst_field}' '{note[dst_field]}'")
                collection.save()
                break
