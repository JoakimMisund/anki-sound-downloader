...
class NaverTTS:
    ...
    def query(self, word, file_format="tts_{word}"):
        ...
        if request.response.headers['content-encoding'] == "gzip":
            ...
            response_text = zlib.decompress(request.response.body, 16+zlib.MAX_WBITS).decode("utf-8")
        else:
        ...
        if (os.path.getsize(filename) == 0):
            print("Error downloading file " + request.url + " to " + filename, file=sys.stderr)
            dest_filename = None
